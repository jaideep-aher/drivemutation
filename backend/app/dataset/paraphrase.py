...
# Family → list of goal templates. `{variant}` filled deterministically.
...
def _load_dotenv_local() -> None:
    """Load ANTHROPIC_API_KEY from .env.local if present (never invent keys)."""
...
def paraphrase_goal(
    ...
    *,
    ...
    """Return a natural-language testing goal.

    If ANTHROPIC_API_KEY is present in the environment / .env.local, optionally
    ask Claude to paraphrase a deterministic base template. Failures fall back
    to the template. Canonical targets are NEVER produced by the LLM.
    """
    if impossible:
    ...
    except Exception:  # noqa: BLE001 — optional path; never block generation
        return base


def _claude_paraphrase(text: str) -> str:
    """Paraphrase user-facing goal text only. No labels/numbers/mutations."""
    import json
